...
#define LD3320_SPI_DEVICE_NAME "spi10"
...
static error_t rt_hw_ld3320_init(void)
{
    ...
    if (RT_EOK == rt_hw_spi_device_attach("spi1", "spi10", GPIOA, GPIO_PIN_4))
    {
        return RT_EOK;
    }
    return RT_ERROR;
...
int rt_ld3320_init(void)
{
    ...
    spi_dev_ld3320 = (struct rt_spi_device *)rt_device_find(LD3320_SPI_DEVICE_NAME);
    if (!spi_dev_ld3320)
    {
        rt_kprintf("spi sample run failed! can't find %s device!\n", LD3320_SPI_DEVICE_NAME);
    }
    else
    {
    ...
    {
        rt_kprintf("spi1 config error\r\n");
    }
    rt_kprintf("spi1 config success\r\n");
